...
const btnWrite = document.querySelector(".todos");
const todoList = document.querySelector(".todo-list");
const total = document.querySelector(".total");
const loadings = document.querySelector(".wait");
const finihs = document.querySelector(".finish");
...
function createdTask(task) {
  ...
  const taskMarkUp = `<div>
              <input type="checkbox" name="" id="${task.id}" ${task.mission ? "checked" : " "}/>
               <span  ${!task.mission ? "contenteditable" : ""}>${task.name}</span>
               </div>
...
function countTask() {
  const complecatedTaskArray = tasks.filter((task) => {
    task.mission === true;
  });
  total.textContent = tasks.length;
  finihs.textContent = complecatedTaskArray.length;
  loadings.textContent = tasks.length - complecatedTaskArray.length;
}
